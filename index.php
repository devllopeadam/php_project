...
try {
  ...
  $loged = isset($_SESSION['username']);
  $stmt = $pdo->prepare("SELECT posts.title, posts.id, posts.image, posts.created_at, users.username, users.profile_image, users.user_title FROM posts 
                        INNER JOIN users
        ...
        echo '
              <div class="min-h-[384px] group justify-between flex py-5 flex-col gap-5 cursor-pointe border-[0.5px] bg-[#21242b] transition-all duration-300 hover:border-[#cacbceda] border-[#8a8d93da] rounded-2xl post-field">
                <div class="flex flex-col gap-5">
                  ...
                  <a href=../php_project/posts_operations/blog.php?post_id=' . $post['id'] . ' class="px-4 flex flex-col gap-1">
                ...
                <div class="px-3">
                  <a href=../php_project/posts_operations/blog.php?post_id=' . $post['id'] . ' class="flex h-[170px] w-full items-center justify-center overflow-hidden rounded-xl">
                  ...
                  ' . (
          $loged
          ? '<a href="../php_project/posts_operations/like.php?post_id=' . $post['id'] . '&user_id=' . $_SESSION['user_id'] . '" class="like px-3 mt-5 flex items-center justify-center gap-2 bg-[#35393d] w-fit py-2 mx-auto rounded-md">
                                                <p class="' . ($isLiked ? 'text-[#ce3df3]' : 'text-[#fc538d]') . '">' . ($isLiked ? 'Dislike' : 'Like') . '</p>
                                                <p id="like" class="' . ($isLiked ? 'text-[#ce3df3]' : 'text-[#fc538d]') . '"text-[#fc538d] cursor-pointer select-none">' . getLikesCount($pdo, $post['id']) . '</p>
                                          </a>'
          : ""
        ) . '
                </div>
